...
\begin{aligned}
(x*o, y_o, z_o) = (x_i, y_i, t*\theta(x*i, y_i) + z_i \cdot exp(s*\theta(x*i, y_i)))
\end{aligned}
Note that its inverse is
\begin{aligned}
(x_i, y_i, z_i) = (x_o, y_o, (z_o - t*\theta(x*o, y_o)) \cdot exp(-s*\theta(x_o, y_o))
\end{aligned}
...
\begin{aligned}
g^m(\mathbf{x}) = \Vert \phi*\theta^{-1}(\mathbf{x};C_m)\Vert - r
\end{aligned}
where $r$ is a radius of sphere. Note that if we let $G$ by
\begin{aligned}
G(\mathbf{x}) = \min*{1 \le m \le n_p} g^m(\mathbf{x})
\end{aligned}
...
\begin{aligned}
L = w*{rec}L*{rec} + w*{occ}L*{occ} + w*{norm}L*{norm} + \\w*{overlap}L*{overlap} + w*{cover}L*{cover}
\end{aligned}
...
\begin{aligned}
max\left(0, \sum\_{m = 1}^{n_p} \sigma(-g^m(\mathbf{x})/\tau) - \lambda\right)
\end{aligned}
...
\begin{aligned}
\sum*{m = 1}^{n_p}\sum*{\mathbf{x}} -\log(10^{-6} + \sigma(-g^m(\mathbf{x})/\tau))
\end{aligned}
