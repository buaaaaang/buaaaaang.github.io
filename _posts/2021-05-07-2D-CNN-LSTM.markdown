...
Validation accuracy was 043, which was not as good as expected

&nbsp;&nbsp;At this point, I pointed out suspicious part of this paper. The input data, which has size (128&times;251&times;1), pass through 4 LFLB layers, and become (1&times;1&times;128). Then, only way to use LSTM layer is putting reshaping layer, which makes data to be (128). This vector is just a sequence of channel of CNN, which does not contain time information, so there is no meaning of using LSTM layer. To check this, I replaced LSTM layer with two dense layers. Here is the result.
